...
class GeminiAdapter:
    ...
    def get_access_token(self):
        """OAuth2 리프레시 토큰을 사용하여 새로운 액세스 토큰 발급"""
        resp = requests.post(
    ...
    async def generate_response(self, prompt: str) -> str:
        try:
            ...
            # OAuth2 기반 Gemini API 호출을 executor에서 실행
            loop = asyncio.get_event_loop()
            response = await loop.run_in_executor(
                None, 
                lambda: self.call_gemini(system_prompt)
            )
            ...
            return f"[ERROR] Gemini API 호출 실패: {str(e)}"
    ...
    async def conduct_team_discussion(self, question: str) -> Dict[str, Any]:
        ...
        try:
            # 각 역할별로 순차적으로 API 호출 (Rate limit 방지)
